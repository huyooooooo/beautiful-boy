...
clear all
clc
global A D  %两个函数公用的变量

%% 导入城市位置
load city_location

%% 计算相互城市间距离
distance = dist(citys,citys');   %矩阵A行向量和矩阵B列向量的欧式距离
...
delta = 2 * rand(N,N) - 1;
U = U0 * log(N-1) + delta;
V = (1 + tansig(U/U0))/2;%神经网络层传递函数，净输入转化为净输出
iter_num = 10000;
E = zeros(1,iter_num);
...
C = sum(V1,1);
R = sum(V1,2);
flag = isequal(C,ones(1,N)) & isequal(R',ones(1,N));%判定数组容量是否相等
...
if flag == 1
   ...
   text(citys_end(1,1),citys_end(1,2),['       起点' ])%指定显示字符串
