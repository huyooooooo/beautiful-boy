...
% 读入图像，放在矩阵pic
pic = imread('littleduck.jpg');%从图形文件读取图像
% 查看矩阵pic的大小和类型
whos pic;%当前工作间所有变量

scrsz = get(0,'ScreenSize');    %get对象
...
model = fitcsvm(TrainLabel, TrainData, '-t 1 -d 1');
%% 进行预测i.e.进行图像分割
...
ind = reshape([TestLabal,TestLabal,TestLabal],m,n,k);%讲矩阵返回成mxnxk的矩阵
ind = logical(ind);%非0数变成1
pic_seg = pic;
pic_seg(~ind) = 0;%非
