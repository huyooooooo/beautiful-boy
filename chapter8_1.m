...
t_test=t(13,:);
%% 交叉验证
desired_spread=[];  %空矩阵，用于初始化
mse_max=10e20;
desired_input=[];
desired_output=[];
result_perfp=[];
indices = crossvalind('Kfold',length(p_train),4);   %交叉函数   矩阵长度
...
save best desired_input desired_output p_test t_test grnn_error mint maxt%保存数据
